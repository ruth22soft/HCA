...
export const authenticate = async (req, res, next) => {
  try {
    ...
    console.log('Full request headers:', req.headers); // Log all headers
    ...
    console.log('User found:', { id: user._id, role: user.role, accountStatus: user.accountStatus });

    // Check if user account is active (default to active if status not set)
    if (user.accountStatus && user.accountStatus !== 'active') {
      console.log('User account is not active:', { id: user._id, status: user.accountStatus });
